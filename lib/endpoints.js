...
const debug = require("debug")("GoKabootar:main");
...
app.get(`${config.baseUrl}file-stats/:fid`, async (req, res, next) => {
  try {
    const info = await store.info(req.params.fid);
    res.json({
      downloadCount: info.metadata.downloadCount || 0,
      downloadLimit: info.metadata.downloadLimit || null,
      expired:
        info.metadata.downloadLimit &&
        (info.metadata.downloadCount || 0) >= info.metadata.downloadLimit,
    });
  } catch (e) {
...
app.get(`${config.baseUrl}files/:fid`, async (req, res, next) => {
  ...
  try {
    ...
    if (
      info.metadata.downloadLimit &&
      info.metadata.downloadCount >= info.metadata.downloadLimit
    ) {
      return res.status(410).send(
        errorPage({
          ...pugVars,
          error: "Download limit reached. This link has expired.",
          lang: req.translations,
          uploadAppPath: config.uploadAppPath || config.baseUrl,
        }),
      );
    ...
    // remove one-time files after download or track download count
    res.on("finish", async () => {
      // Increment download count if limit is set
      if (info.metadata.downloadLimit) {
        const newCount = (info.metadata.downloadCount || 0) + 1;
        await db.updateMetadata(info.metadata.sid, info.metadata.key, {
          downloadCount: newCount,
        });

        // Remove if limit reached
        if (newCount >= info.metadata.downloadLimit) {
          await db.remove(info.metadata.sid, info.metadata.key);
        }
      } else if (info.metadata.retention === "one-time") {
        await db.remove(info.metadata.sid, info.metadata.key);
      } else {
        await db.updateLastDownload(info.metadata.sid, info.metadata.key);
      }
